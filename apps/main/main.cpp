...
public:
  ...
  names(std::vector<std::string> nets,
    ...
    // Construct gate from given so_cover to make simulation of so_cover
    // possible
    auto gate = gate_ns::make_shared(coverName);
    gate->setLogicFunction(
        logic_function_ns::LogicFunction<UserDefinedLogicFunctionType>(
            truthTable));
     ...
     * Attach output net
     */
    gate->attachNet(netlist_.findNet(nets[outputNet]),
                    net_ns::NetType::OUTPUT_NET);
  }
