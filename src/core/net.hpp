...
struct NetTypeBitmap final {
  /*
   * Don't mark this constructor as explicit to make possbiel construction of
   * bitmap from a single value
   */
  ...
  bool operator==(const NetTypeBitmap &other);
  bool operator!=(const NetTypeBitmap &other);
...
static const auto PrimaryInputBitmask = NetTypeBitmap(NetType::INPUT_NET);
static const auto PrimaryOutputBitmask = NetTypeBitmap(NetType::OUTPUT_NET);

struct IsPrimaryInputPerdicate {
  bool operator()(NetSPtr net) {
    return net->getTypeBitmap() == PrimaryInputBitmask;
  }
};

struct IsPrimaryOutputPerdicate {
  bool operator()(NetSPtr net) {
    return net->getTypeBitmap() == PrimaryOutputBitmask;
  }
};
