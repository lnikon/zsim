...
struct Netlist {
  ...
  gate_ns::GateSPtr addGate(NameBase::name_type_cref name,
                            DelayBase::delay_type delay = 0);
  gate_ns::GateSPtr addGate(gate_ns::GateSPtr gate);

  net_ns::NetSPtr addNet(NameBase::name_type_cref name, net_ns::NetType type,
                         DelayBase::delay_type delay = 0);

  void attachNet(NameBase::name_type_cref gateName,
...
private:
   ...
   */
  std::unordered_map<NameBase::name_type,
                     LogicFunction<UserDefinedLogicFunctionType>>
      so_covers_;
};
