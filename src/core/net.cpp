...
net_ns::Net::Net(NameBase::name_type_cref name, net_ns::NetType type, DelayBase::delay_type delay)
...
net_ns::Net::Net(NameBase::name_type_cref name)
    : NodeBase{name}
{
    reset();
}
...
void net_ns::Net::setType(net_ns::NetType type)
{
    bitmap_.set(type);
}

net_ns::NetTypeBitmap net_ns::Net::getTypeBitmap() const
{
    return bitmap_;
}
...
void net_ns::Net::addGate(gate_ns::GateSPtr gate)
{
    gates_.emplace(gate);
}

void net_ns::Net::reset()
{
    bitmap_.reset();
}
...
{
    for(uint32_t typeIdx = 0; typeIdx < static_cast<uint32_t>(NetType::ENUM_END); ++typeIdx)
    {
